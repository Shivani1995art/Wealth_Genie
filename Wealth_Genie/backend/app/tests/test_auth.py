...
def test_verify_token_rs256_mocked():
    ...
    # 2. Generate a valid RS256 token
    user_id = str(uuid4())
    ...
    # 3. Patch jwks_client.get_signing_key_from_jwt to return our public key
...
def test_verify_token_es256_mocked():
    # 1. Generate EC key pair for testing (P-256 curve)
    private_key = ec.generate_private_key(ec.SECP256R1())
    public_key = private_key.public_key()
    
    # 2. Generate a valid ES256 token
    user_id = str(uuid4())
    ...
    # Encode with ES256 using private key
    pem_private = private_key.private_bytes(
        encoding=serialization.Encoding.PEM,
